...
function MaleAvatar({ size=64 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 200 260" xmlns="http:
...
function UnknownAvatar({ size=64 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" xmlns="http:
...
const S = {
  ...
  btnPrimary:{ background:"linear-gradient(135deg,#2E7D32,#1B5E20)", border:"none", borderRadius:10, padding:"12px 24px", color:"#fff", fontSize:16, fontWeight:"700", cursor:"pointer", fontFamily:FONT, boxShadow:"0 2px 8px rgba(27,94,32,0.3)" },
  btnGold:{ background:"linear-gradient(135deg,#F9A825,#9E6C00)", border:"none", borderRadius:10, padding:"12px 24px", color:"#fff", fontSize:16, fontWeight:"700", cursor:"pointer", fontFamily:FONT, boxShadow:"0 2px 8px rgba(158,108,0,0.25)" },
...
function BackButton({ onBack, label="← Back to Dashboard" }) {
  return (
    <button onClick={onBack} style={{display:"flex",alignItems:"center",gap:8,background:"linear-gradient(135deg,#2E7D32,#1B5E20)",border:"none",borderRadius:10,padding:"10px 20px",color:"#fff",fontSize:15,fontWeight:"700",cursor:"pointer",fontFamily:FONT,boxShadow:"0 2px 8px rgba(27,94,32,0.25)"}}>
      ← {label}
    </button>
  );
}
function AnnouncementBadge({ announcements, isAdmin, onAdd, onDelete, onEdit }) {
  ...
  const startEdit = (realIdx, a) => { setEditIdx(realIdx); setEditTitle(a.title); setEditText(a.text); };
  ...
  return (
    <div style={{position:"relative"}}>
      ...
      {open && (
        <div style={{position:"absolute", top:60, right:0, width:380, background:"#fff", border:`1.5px solid ${C.border}`, borderRadius:16, boxShadow:"0 8px 32px rgba(0,0,0,0.12)", zIndex:1000, overflow:"hidden"}}>
          ...
          {isAdmin && (
            <div style={{padding:"14px 18px", borderBottom:`1px solid ${C.border}`, background:"#fffdf8"}}>
              ...
              <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Title (e.g. Holiday Notice)" style={{...S.inputSm, width:"100%", boxSizing:"border-box", marginBottom:8}}/>
              <textarea value={draft} onChange={e=>setDraft(e.target.value)} placeholder="Write your announcement here..." style={{...S.inputSm, width:"100%", boxSizing:"border-box", resize:"vertical", minHeight:70, lineHeight:1.5, marginBottom:10}}/>
                    ...
                    <div key={i} style={{padding:"14px 18px", borderBottom:`1px solid ${C.border}`, background:a.read?"#fff":"#fffbf0"}}>
                      {isEditing ? (
                        <div style={{display:"flex",flexDirection:"column",gap:8}}>
                          ...
                          <textarea value={editText} onChange={e=>setEditText(e.target.value)} style={{...S.inputSm,width:"100%",boxSizing:"border-box",resize:"vertical",minHeight:60,lineHeight:1.5}}/>
                        ...
                        <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8}}>
                          ...
                          {isAdmin && (
                            <div style={{display:"flex",gap:6,flexShrink:0}}>
                              <button onClick={()=>startEdit(realIdx,a)} style={{background:"none",border:"none",color:C.gold,cursor:"pointer",fontSize:15,padding:"2px 5px"}}>✏️</button>
                              <button onClick={()=>onDelete(realIdx)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:15,padding:"2px 5px"}}>🗑️</button>
                            </div>
                          )}
                        </div>
                      )}
...
function AnnouncementBoard({announcements,isAdmin,onMarkAllRead,onDelete,onEdit}){
  ...
  const al=unread>0;
  const startEdit=(realIdx,a)=>{setEditIdx(realIdx);setEditTitle(a.title);setEditText(a.text);};
  const saveEdit=async()=>{await onEdit(editIdx,{...announcements[editIdx],title:editTitle.trim(),text:editText.trim()});setEditIdx(null);};
  return(
          ...
          {sorted.map((a,i)=>{
            ...
            return(
              <div key={i} style={{background:a.read?"#fff":al?"#fff5f5":"#f0fff0",border:`2px solid ${a.read?C.border:al?"#EF9A9A":"#81C784"}`,borderLeft:`6px solid ${a.read?C.borderMid:al?"#D32F2F":"#2E7D32"}`,borderRadius:12,padding:"14px 16px",position:"relative"}}>
                {!a.read&&!isEditing&&<span style={{position:"absolute",top:10,right:12,background:al?"#D32F2F":C.primary,color:"#fff",fontSize:11,fontWeight:"bold",borderRadius:10,padding:"2px 9px",letterSpacing:1}}>NEW</span>}
                {isEditing?(
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    <input value={editTitle} onChange={e=>setEditTitle(e.target.value)} style={{...S.inputSm,width:"100%",boxSizing:"border-box"}}/>
                    <textarea value={editText} onChange={e=>setEditText(e.target.value)} style={{...S.inputSm,width:"100%",boxSizing:"border-box",resize:"vertical",minHeight:70,lineHeight:1.5}}/>
                    <div style={{display:"flex",gap:8}}>
                  ...
                  <>
                    ...
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:6}}>
                      ...
                      {isAdmin&&<div style={{display:"flex",gap:8}}>
                        <button onClick={()=>startEdit(realIdx,a)} style={{background:"none",border:`1px solid ${C.gold}`,borderRadius:8,padding:"4px 12px",color:C.gold,cursor:"pointer",fontSize:13,fontFamily:FONT}}>✏️ Edit</button>
...
function WeekTable({ weekData, editing, onCellChange, onFeedbackChange, onSaveSection, isAdmin }) {
  const [editingSection, setEditingSection] = useState(null);
  const [localCells, setLocalCells] = useState(null);
  const [localFeedback, setLocalFeedback] = useState("");
  const startEdit = (section) => {
    setEditingSection(section);
    if(section==="feedback") setLocalFeedback(weekData.teacherFeedback||"");
    else setLocalCells(weekData.days.map(d=>d[section]||""));
  };
  const cancelEdit = () => { setEditingSection(null); setLocalCells(null); };
  const saveSection = (section) => {
    if(section==="feedback") onSaveSection("feedback", null, localFeedback);
    else onSaveSection("cell", section, localCells);
    setEditingSection(null);
  };
  const deleteSection = (section) => {
    if(section==="feedback") onSaveSection("feedback", null, "");
    else onSaveSection("cell", section, weekData.days.map(()=>""));
  };
  const fieldLabel = (f) => f==="sabq"?"📖 Sabq":f==="manzil"?"🔁 Manzil":"📝 Notes";
  return (
    <div style={{border:`1.5px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:15}}>
          <thead>
            <tr style={{background:"#E8F5E9"}}>
              <th style={{padding:"11px 14px",textAlign:"left",color:C.textMid,fontWeight:"bold",fontSize:13,letterSpacing:1,borderBottom:`1px solid ${C.border}`,minWidth:130}}>CATEGORY</th>
              ...
              {isAdmin&&<th style={{padding:"11px 14px",textAlign:"center",color:C.textMid,fontWeight:"bold",fontSize:12,borderBottom:`1px solid ${C.border}`,minWidth:130}}>ACTIONS</th>}
            </tr>
          </thead>
          <tbody>
            {["sabq","manzil","notes"].map((field,fi)=>{
              const isEditingThis = editingSection===field;
              return (
                <tr key={field} style={{background:fi%2===0?"#fafcfa":"#fff"}}>
                  <td style={{padding:"10px 14px",color:C.text,fontSize:14,fontWeight:"bold",borderBottom:`1px solid ${C.border}`,whiteSpace:"nowrap"}}>{fieldLabel(field)}</td>
                    ...
                    <td style={{padding:"7px 10px",borderBottom:`1px solid ${C.border}`,textAlign:"center"}}>
                      {isEditingThis?(
                        <div style={{display:"flex",gap:5,justifyContent:"center",flexWrap:"wrap"}}>
                          <button onClick={()=>saveSection(field)} style={{...S.btnPrimary,fontSize:12,padding:"5px 12px"}}>💾 Save</button>
                          <button onClick={cancelEdit} style={{...S.btnGhost,fontSize:12,padding:"5px 10px"}}>✕</button>
                        </div>
                      ):(
                        <div style={{display:"flex",gap:5,justifyContent:"center",flexWrap:"wrap"}}>
                          <button onClick={()=>startEdit(field)} style={{...S.btnGold,fontSize:12,padding:"5px 12px"}}>✏️ Edit</button>
                          <button onClick={()=>deleteSection(field)} style={{...S.btnDanger,fontSize:12,padding:"5px 10px"}}>🗑</button>
                        </div>
                      )}
                    </td>
                  )}
      ...
      {/* Teacher Weekly Feedback with Save/Edit/Delete */}
      <div style={{padding:"14px 18px",background:"#f8fcf8",borderTop:`1px solid ${C.border}`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,flexWrap:"wrap",gap:8}}>
          <p style={{color:C.primary,fontSize:13,fontWeight:"bold",letterSpacing:1,margin:0}}>✍️ TEACHER WEEKLY FEEDBACK</p>
          {isAdmin&&editingSection!=="feedback"&&(
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>startEdit("feedback")} style={{...S.btnGold,fontSize:12,padding:"5px 14px"}}>✏️ Edit</button>
              <button onClick={()=>deleteSection("feedback")} style={{...S.btnDanger,fontSize:12,padding:"5px 12px"}}>🗑 Delete</button>
            </div>
          )}
        </div>
        {editingSection==="feedback"?(
          <div>
            <textarea value={localFeedback} onChange={e=>setLocalFeedback(e.target.value)} placeholder="Enter weekly feedback..." style={{...S.inputSm,width:"100%",boxSizing:"border-box",resize:"vertical",minHeight:75,lineHeight:1.6,marginBottom:10}}/>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>saveSection("feedback")} style={{...S.btnPrimary,fontSize:13,padding:"7px 18px"}}>💾 Save Feedback</button>
              <button onClick={cancelEdit} style={{...S.btnGhost,fontSize:13,padding:"7px 14px"}}>Cancel</button>
            </div>
          </div>
        ):(
...
function MessageSection({ student, isAdmin, onSave }) {
  ...
  const saveEdit = async () => {
    ...
    await onSave({...student,messages:newMsgs});
    setEditIdx(null);
  };
    ...
    <div style={S.card}>
      ...
      <div style={{background:"#f8fcf8",border:`1px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:16,minHeight:130,maxHeight:320,overflowY:"auto",display:"flex",flexDirection:"column",gap:10}}>
        {monthMsgs.length===0
          ...
          :monthMsgs.map((m)=>{
            const isMyMsg = isAdmin?m.from==="teacher":m.from==="parent";
            const isEditing = editIdx===m._idx;
            return(
              <div key={m._idx} style={{alignSelf:m.from==="parent"?"flex-end":"flex-start",background:m.from==="parent"?"#e0f0e0":"#fff8e8",border:`1px solid ${m.from==="parent"?C.border:"#e0d090"}`,borderRadius:12,padding:"11px 16px",maxWidth:"82%",position:"relative"}}>
                {isEditing?(
                  ...
                  <>
                    ...
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
                      ...
                      {isMyMsg&&isAdmin&&(
                        <div style={{display:"flex",gap:6}}>
                          <button onClick={()=>{setEditIdx(m._idx);setEditText(m.text);}} style={{background:"none",border:"none",color:C.gold,cursor:"pointer",fontSize:13,padding:"1px 4px"}}>✏️</button>
                          <button onClick={()=>deleteMsg(m._idx)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:13,padding:"1px 4px"}}>🗑️</button>
                        </div>
                      )}
                    </div>
                  </>
          ...
          return (
            <div key={m} style={{display:"flex",alignItems:"center",gap:12,background:paid===true?"#edf8ed":paid===false?"#fdf0f0":"#f8f8f8",border:`1.5px solid ${paid===true?C.success:paid===false?C.danger:C.border}`,borderRadius:10,padding:"10px 16px",flexWrap:"wrap"}}>
              ...
              {isAdmin&&<div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                ...
                <button onClick={()=>reset(m)} style={{background:"#f0f0f0",border:`1px solid ${C.border}`,borderRadius:8,padding:"6px 12px",fontSize:13,color:C.textMid,cursor:"pointer",fontFamily:FONT}} disabled={paid===null}>↺ Reset</button>
              </div>}
            </div>
          );
        })}
...
function StudentDetail({ student, onBack, isAdmin, isParent, onSave }) {
  ...
  const feedbackChange = (val)=>{ const u=JSON.parse(JSON.stringify(local)); if(!u.quranProgress) u.quranProgress=EMPTY_PROGRESS(); u.quranProgress[selYear][selMonth][activeWeek].teacherFeedback=val; const ts=new Date().toLocaleString("en-AU"); u.parentNotifs=[...(u.parentNotifs||[]),{text:`📝 Teacher added feedback — ${local.name} ${selMonth} Week ${activeWeek+1}`,time:ts,read:false}]; setLocal(u); };
  ...
  const handleSaveSection = async (type, field, value) => {
    ...
    if(type==="feedback"){
      ...
      if(value) { const ts=new Date().toLocaleString("en-AU"); u.parentNotifs=[...(u.parentNotifs||[]),{text:`📝 Teacher added feedback — ${local.name} ${selMonth} Week ${activeWeek+1}`,time:ts,read:false}]; }
    } else {
  ...
  const saveSummarySection = async (key) => { await onSave(local); };
  const toggleDisable = async () => {
    const u = {...local, disabled: !local.disabled};
    setLocal(u);
    await onSave(u);
  };
  const deleteStudent = async () => {
    const u = makeStudent(local.id, "", "");
    await onSave(u);
    ...
    <div>
      ...
      {local.disabled&&(
        <div style={{background:"#fff3e0",border:"2px solid #e65100",borderRadius:12,padding:"12px 18px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
          ...
          <p style={{color:"#e65100",fontSize:15,fontWeight:"bold",margin:0}}>This student is currently DISABLED. Restore to re-enable access.</p>
        </div>
      )}
      {/* Student header card */}
      ...
      {isAdmin&&(
        <div style={{marginBottom:18,display:"flex",gap:10,flexWrap:"wrap"}}>
          ...
          <button onClick={toggleDisable} style={{...local.disabled?S.btnSuccess:S.btnWarning,fontSize:14,padding:"10px 18px"}}>
            {local.disabled?"✅ Enable Student":"🚫 Disable Student"}
          </button>
          {/* Delete */}
          {!showDeleteConfirm
            ?<button onClick={()=>setShowDeleteConfirm(true)} style={{...S.btnDanger,fontSize:14,padding:"10px 18px"}}>🗑️ Delete Student</button>
            :<div style={{display:"flex",alignItems:"center",gap:8,background:"#fff5f5",border:"1.5px solid #ef9a9a",borderRadius:10,padding:"8px 14px"}}>
              <span style={{color:C.danger,fontSize:14,fontWeight:"bold"}}>Are you sure?</span>
              <button onClick={deleteStudent} style={{...S.btnDanger,fontSize:13,padding:"6px 14px"}}>Yes, Delete</button>
              <button onClick={()=>setShowDeleteConfirm(false)} style={{...S.btnGhost,fontSize:13,padding:"6px 12px"}}>Cancel</button>
            </div>
          }
        </div>
      )}
      ...
      {tab==="progress"&&(
        <>
          ...
          <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
            {getWeeks().map((w,i)=>(
              <button key={i} onClick={()=>setActiveWeek(i)} style={{padding:"9px 20px",borderRadius:22,border:activeWeek===i?"2px solid #2E7D32":"2px solid #C8E6C9",background:activeWeek===i?"#E8F5E9":"#fff",color:activeWeek===i?"#1B5E20":"#4E6B4E",fontSize:15,cursor:"pointer",fontFamily:FONT,fontWeight:activeWeek===i?"bold":"normal"}}>Week {w.week}</button>
            ))}
          </div>
          ...
          {/* Strengths & To Improve with Save/Edit/Delete per section */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
            <SummarySection
              label="✦ STRENGTHS" color={C.success} borderColor={C.success}
              value={local.summary?.strengths||""} isAdmin={isAdmin}
              onChange={v=>summaryChange("strengths",v)}
              onSave={()=>saveSummarySection("strengths")}
              onDelete={()=>{summaryChange("strengths","");onSave({...local,summary:{...local.summary,strengths:""}});}}
            />
            <SummarySection
              label="✦ TO IMPROVE" color="#b07020" borderColor="#e0a030"
              value={local.summary?.improve||""} isAdmin={isAdmin}
              onChange={v=>summaryChange("improve",v)}
              onSave={()=>saveSummarySection("improve")}
              onDelete={()=>{summaryChange("improve","");onSave({...local,summary:{...local.summary,improve:""}});}}
            />
          </div>
        </>
      )}
  ...
  return (
    <div style={{...S.card,borderLeft:`4px solid ${borderColor}`}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9,flexWrap:"wrap",gap:6}}>
        ...
        {isAdmin&&!editing&&(
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>setEditing(true)} style={{...S.btnGold,fontSize:11,padding:"4px 10px"}}>✏️ Edit</button>
...
function StudentCard({ student, onSelect, unreadCount=0 }) {
  return (
    <button onClick={()=>onSelect(student)} style={{background:student.disabled?"#f5f5f5":"#fff",border:`1.5px solid ${student.disabled?"#e0e0e0":"#C8E6C9"}`,borderRadius:16,padding:20,cursor:"pointer",textAlign:"left",fontFamily:FONT,display:"flex",alignItems:"center",gap:16,width:"100%",boxShadow:"0 2px 10px rgba(0,0,0,0.05)",position:"relative",transition:"all 0.15s",opacity:student.disabled?0.65:1}}
      onMouseEnter={e=>{if(!student.disabled){e.currentTarget.style.borderColor="#2E7D32";e.currentTarget.style.boxShadow="0 6px 24px rgba(27,94,32,0.12)";e.currentTarget.style.transform="translateY(-2px)";}}}
      onMouseLeave={e=>{e.currentTarget.style.borderColor=student.disabled?"#e0e0e0":"#C8E6C9";e.currentTarget.style.boxShadow="0 2px 10px rgba(0,0,0,0.05)";e.currentTarget.style.transform="translateY(0)";}}>
      {student.disabled&&<span style={{position:"absolute",top:10,left:12,background:"#e65100",color:"#fff",fontSize:10,fontWeight:"bold",borderRadius:8,padding:"2px 8px",letterSpacing:1}}>DISABLED</span>}
...
function AccountManager({ accounts, onSave, students }) {
  ...
  const saveAll = async (updated) => {
    ...
    await onSave([adminAcc,...updated]);
    setSaving(false);
  };
  const updateField = (i,f,v) => { const u=[...list]; u[i]={...u[i],[f]:v}; setList(u); };
  const toggleStudent = (i,sid) => {
    const u=[...list];
    const ids=u[i].studentIds||[];
    u[i]={...u[i],studentIds:ids.includes(sid)?ids.filter(x=>x!==sid):[...ids,sid]};
    setList(u);
  };
  const deleteAcc = (i) => { if(!window.confirm("Delete this account?")) return; const u=list.filter((_,idx)=>idx!==i); setList(u); saveAll(u); };
  const addAccount = async () => {
    if(!newAcc.login.trim()||!newAcc.password.trim()) return;
    const u=[...list,{...newAcc,login:newAcc.login.trim().toLowerCase(),role:"parent"}];
    setList(u); await saveAll(u); setNewAcc({login:"",password:"",studentIds:[]}); setShowNew(false);
  };
  const toggleNewStudent = (sid) => {
    const ids=newAcc.studentIds||[];
    setNewAcc(p=>({...p,studentIds:ids.includes(sid)?ids.filter(x=>x!==sid):[...ids,sid]}));
  };
  const namedStudents = students.filter(s=>s.name);
  return (
    <div style={S.card}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:10}}>
        <p style={{color:C.primary,fontSize:16,fontWeight:"bold",letterSpacing:1,margin:0}}>🔑 PARENT ACCOUNTS</p>
      ...
      {showNew&&(
        <div style={{background:"#fffdf0",border:`1.5px solid ${C.goldLight}`,borderRadius:12,padding:"16px 18px",marginBottom:18}}>
          <p style={{color:C.gold,fontSize:14,fontWeight:"bold",margin:"0 0 12px"}}>NEW ACCOUNT</p>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:10}}>
            <input value={newAcc.login} onChange={e=>setNewAcc(p=>({...p,login:e.target.value}))} placeholder="Login (e.g. parent.name@darulnoor)" style={{...S.inputSm,flex:2,minWidth:200}}/>
            <input value={newAcc.password} onChange={e=>setNewAcc(p=>({...p,password:e.target.value}))} placeholder="Password" style={{...S.inputSm,flex:1,minWidth:120}}/>
          </div>
          <p style={{color:C.textMid,fontSize:13,fontWeight:"bold",margin:"0 0 8px"}}>Assign students:</p>
          <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12}}>
            {namedStudents.map(s=>(
              <label key={s.id} style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",background:newAcc.studentIds.includes(s.id)?"#e8f5e9":"#f5f5f5",border:`1px solid ${newAcc.studentIds.includes(s.id)?C.primary:C.border}`,borderRadius:8,padding:"5px 10px",fontSize:13}}>
                <input type="checkbox" checked={newAcc.studentIds.includes(s.id)} onChange={()=>toggleNewStudent(s.id)} style={{width:14,height:14}}/>{s.name}
              </label>
            ))}
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={addAccount} disabled={!newAcc.login.trim()||!newAcc.password.trim()} style={{...S.btnPrimary,fontSize:13,padding:"7px 18px",opacity:(!newAcc.login.trim()||!newAcc.password.trim())?0.5:1}}>✅ Create Account</button>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {list.map((acc,i)=>{
          ...
          const assignedNames=(acc.studentIds||[]).map(id=>students.find(s=>s.id===id)?.name).filter(Boolean);
          return(
            <div key={i} style={{background:isEditing?"#f8fff8":"#fafafa",border:`1.5px solid ${isEditing?C.primary:C.border}`,borderRadius:12,padding:"14px 16px"}}>
              {isEditing?(
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                    <input value={list[i].login} onChange={e=>updateField(i,"login",e.target.value)} placeholder="Login" style={{...S.inputSm,flex:2,minWidth:180}}/>
                    <input value={list[i].password} onChange={e=>updateField(i,"password",e.target.value)} placeholder="Password" style={{...S.inputSm,flex:1,minWidth:120}}/>
                  </div>
                  <p style={{color:C.textMid,fontSize:13,fontWeight:"bold",margin:"4px 0 6px"}}>Assigned students:</p>
                  <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:8}}>
                    {namedStudents.map(s=>(
                      <label key={s.id} style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",background:list[i].studentIds?.includes(s.id)?"#e8f5e9":"#f5f5f5",border:`1px solid ${list[i].studentIds?.includes(s.id)?C.primary:C.border}`,borderRadius:8,padding:"5px 10px",fontSize:13}}>
                        <input type="checkbox" checked={!!(list[i].studentIds?.includes(s.id))} onChange={()=>toggleStudent(i,s.id)} style={{width:14,height:14}}/>{s.name}
                      </label>
                    ))}
                  </div>
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={async()=>{setSaving(true);await saveAll(list);setSaving(false);setEditIdx(null);}} style={{...S.btnPrimary,fontSize:13,padding:"7px 18px"}}>{saving?"Saving...":"💾 Save"}</button>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                  <div>
                    <p style={{color:C.text,fontSize:15,fontWeight:"bold",margin:"0 0 4px"}}>👤 {acc.login}</p>
                    <p style={{color:C.textMid,fontSize:13,margin:"0 0 4px"}}>
                    ...
                    <p style={{color:C.textLight,fontSize:12,margin:0}}>Students: {assignedNames.length?assignedNames.join(", "):"None assigned"}</p>
                  </div>
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={()=>setEditIdx(i)} style={{...S.btnGold,fontSize:13,padding:"7px 14px"}}>✏️ Edit</button>
                    <button onClick={()=>deleteAcc(i)} style={{...S.btnDanger,fontSize:13,padding:"7px 12px"}}>🗑️</button>
                  </div>
                </div>
              )}
            </div>
  ...
  return (
    <div>
      ...
      <div style={{marginBottom:16}}>
        <button onClick={()=>setShowAccounts(p=>!p)} style={{...S.btnGhost,fontSize:14,padding:"9px 18px",display:"flex",alignItems:"center",gap:8}}>
                ...
                return(
                  <div key={group.label} style={{background:"#F1F8F1",border:"1.5px solid #C8E6C9",borderRadius:16,padding:"18px 16px 14px"}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:8}}>
                      ...
                      <button onClick={()=>{ const nextEmpty=students.find(s=>!s.name&&group.ids.includes(s.id)); if(nextEmpty) onSetupSlot(nextEmpty.id,"family"); else alert("No empty slots in this family group. Add more slots in the code."); }} style={{...S.btnGhost,fontSize:13,padding:"6px 14px"}}>+ Add Student</button>
                    </div>
...
function Dashboard({ account, onLogout, accounts, onAccountsUpdate }) {
  ...
  useEffect(()=>{
    ...
    const unsubAnnounce=onSnapshot(announcementsRef,snap=>{ if(snap.exists()) setAnnouncements(snap.data().list||[]); });
  ...
  const addAnnouncement = async (a) => { const nl=[...announcements,a]; await setDoc(doc(db,"config","announcements"),{list:nl}); setAnnouncements(nl); };
  const deleteAnnouncement = async (idx) => { const nl=announcements.filter((_,i)=>i!==idx); await setDoc(doc(db,"config","announcements"),{list:nl}); setAnnouncements(nl); };
  const editAnnouncement = async (idx, updated) => { const nl=announcements.map((a,i)=>i===idx?updated:a); await setDoc(doc(db,"config","announcements"),{list:nl}); setAnnouncements(nl); };
  const markAllAnnouncementsRead = async () => { const nl=announcements.map(a=>({...a,read:true})); await setDoc(doc(db,"config","announcements"),{list:nl}); setAnnouncements(nl); };
  const saveAccounts = async (updated) => {
    await setDoc(doc(db,"config","accounts"),{list:updated});
    onAccountsUpdate(updated);
  };
  const clearAllNotifs = async () => {
    if(isAdmin){ for(const s of students){ if((s.adminNotifs||[]).some(n=>!n.read)){ await handleSave({...s,adminNotifs:(s.adminNotifs||[]).map(n=>({...n,read:true}))}); } } }
    else { for(const s of visibleStudents){ if((s.parentNotifs||[]).some(n=>!n.read)){ await handleSave({...s,parentNotifs:(s.parentNotifs||[]).map(n=>({...n,read:true}))}); } } }
    setShowNotif(false);
  };
  return (
    <div style={{minHeight:"100vh",background:"#F5F7F5",fontFamily:FONT,color:"#1A1A1A"}}>
      <div style={{maxWidth:960,margin:"0 auto",padding:"20px 18px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:26,background:"#fff",borderRadius:16,padding:"16px 22px",border:`1.5px solid ${C.border}`,boxShadow:"0 2px 10px rgba(0,0,0,0.05)",flexWrap:"wrap",gap:12}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:14,marginLeft:"auto"}}>
            ...
            <AnnouncementBadge announcements={announcements} isAdmin={isAdmin} onAdd={addAnnouncement} onDelete={deleteAnnouncement} onEdit={editAnnouncement}/>
            <div style={{position:"relative"}}>
              <button onClick={()=>setShowNotif(p=>!p)} style={{background:unreadCount>0?"#fff8e8":"#f8f8f8",border:`1.5px solid ${unreadCount>0?"#e0c060":C.border}`,borderRadius:12,padding:"10px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:8,fontSize:22,position:"relative"}}>
                🔔
                {unreadCount>0&&<span style={{position:"absolute",top:-8,right:-8,background:C.notif,color:"#fff",borderRadius:"50%",minWidth:22,height:22,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:"bold",fontFamily:FONT,padding:"0 4px"}}>{unreadCount}</span>}
              </button>
        ...
        {!selected&&(
          <AnnouncementBoard announcements={announcements} isAdmin={isAdmin} onMarkAllRead={markAllAnnouncementsRead} onDelete={deleteAnnouncement} onEdit={editAnnouncement}/>
        )}
        {selected?(
          ...
          <>
            ...
            {isAdmin
              ?<AdminView students={visibleStudents} onSelect={setSelected} onSetupSlot={(id)=>setSelected(students.find(s=>s.id===id))} accounts={accounts} onSaveAccounts={saveAccounts}/>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const unsub = onSnapshot(ref, snap=>{
      if(snap.exists()&&snap.data().list?.length) setAccounts(snap.data().list);
      setAccountsLoaded(true);
    });
    const timer = setTimeout(()=>setAccountsLoaded(true), 3000);
    return()=>{ unsub(); clearTimeout(timer); };
  },[]);
  if(!accountsLoaded) return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:FONT,color:C.primary,fontSize:18}}>Loading...</div>;
  return account
    ?<Dashboard account={account} onLogout={()=>setAccount(null)} accounts={accounts} onAccountsUpdate={setAccounts}/>
    :<LoginScreen onLogin={setAccount} accounts={accounts}/>;
}
